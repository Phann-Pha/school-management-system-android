...
class DataStoreManager(context: Context) {
    ...
    inline fun <reified T> getObjectData(key: Preferences.Key<String>): Flow<T> =
        dataStore.data.map { deSerializeData(data = it[key] ?: "") }

    // get primitives data
    fun getStringData(key: Preferences.Key<String>): Flow<String> = dataStore.data.map { it[key] ?: "" }

    fun getIntData(key: Preferences.Key<Int>): Flow<Int> = dataStore.data.map { it[key] ?: 0 }

    fun getDoubleData(key: Preferences.Key<Double>): Flow<Double> = dataStore.data.map { it[key] ?: 0.0 }

    fun getFloatData(key: Preferences.Key<Float>): Flow<Float> = dataStore.data.map { it[key] ?: 0.0f }

    fun getLongData(key: Preferences.Key<Long>): Flow<Long> = dataStore.data.map { it[key] ?: 0 }

    fun getBooleanData(key: Preferences.Key<Boolean>): Flow<Boolean> = dataStore.data.map { it[key] ?: false }

    private fun <T> serializeData(data: T): String = Gson().toJson(data)

    inline fun <reified T> deSerializeData(data: String): T = Gson().fromJson(data, T::class.java)
}